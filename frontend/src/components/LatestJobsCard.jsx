...
const  LatestJobsCard =({job}) => {
  ...
  return (
    <div className='p-5 rounded-md shadow-md md:shadow-lg border border-gray-100' onClick={()=>{
      navigate(`/jobs/description/${job?._id}`);
    }}>
        <div>
        <h1 className=' sm:text-lg font-medium'>{job?.company?.name}</h1>
        <p className='text-sm text-gray-500'>{job?.location}</p>
        </div>
        <div>
            <h1 className='font-bold sm:text-lg my-2'>{job?.title}</h1>
